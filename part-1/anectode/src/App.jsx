...
const Anecdote_display = ({ name, text, votes }) => {
  return (
    <div>
      <h1>{name}</h1>
      <p>{text}</p>
...
const Anecdote = ({ anecdotes }) => {
  ...
  const selectMaximum = () => {
    ...
    return Object.keys(allVotes).find(key => allVotes[key] == maxi)
  }
  return (
    < div >
      <Anecdote_display name={'Anectode of the day'} text={anecdotes[selected]} votes={allVotes[selected]} />
      <Button name={'vote'} handleClick={addVote} />
      <Button name={'next anecdote'} handleClick={selectRandomAnecdote} />
      <Anecdote_display name={'Anectode with most votes'} text={anecdotes[selectMaximum()]} votes={allVotes[selectMaximum()]} />
    </div >
  )
}
